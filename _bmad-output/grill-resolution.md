...
- Accepted reviewer pushback that guided-tour accuracy must prove the current product contract, not just valid sidebar IDs. The tour now targets Command Center bid/ask entry, Market Radar, Marketplace bid/ask entry, orderbook depth, Market Terminal, indicative Forward Curve, activity, alerts, regulations, analytics/trades, and notifications, with regression tests for copy and anchors.
- Resolved the first performance slice around perceived platform navigation latency: lazy screen switches should keep the dashboard shell mounted and use a local content skeleton, while heavier API and bundle work remains a separate measured optimization track.
- Accepted reviewer pushback that Marketplace request pressure is a clearer small win than additional chunk warming. Marketplace listing reads and product-count probes now have a short TTL, in-flight dedupe, manual refresh bypass, and mutation-triggered cache clearing.
- Extended the same request-pressure principle to shared catalog reads: products and delivery points are static enough for short-lived API-client reuse, reducing repeat Terminal, Forward Curve, modal, and map refetches during rapid platform navigation.
- Extended the performance slice to Market Terminal orderbook reads: auth-scoped short-lived freshness and in-flight dedupe avoid repeat loads on remount, while SSE events are debounced before forced refresh so bursts do not trigger one full-orderbook request per event.
- Accepted reviewer pushback that market-intelligence caching must be typed and endpoint-specific rather than a generic GET cache. Price summaries now use 15s token-scoped freshness, reference prices use 60s, forward curves use 10s, manual curve refresh bypasses cache, and local/SSE invalidation distinguishes orderbook-derived curves from trade-derived price reads.
- Accepted reviewer pushback that manual vendor splitting is only useful if global prefetch does not erase it. MapLibre, Leaflet, lightweight-charts, and Recharts now build as separate vendor chunks, and dashboard idle prefetch excludes Map/Terminal/Forward Curve so heavy vendors load on navigation intent instead of every app mount.
- Added an artifact-level build gate after reviewer pushback that source config is not enough. `npm run verify` now inspects `dist/` after production build to confirm split map/chart vendors, absence of legacy grouped chunks, and no initial-HTML eager reference to heavy map/chart vendors.
