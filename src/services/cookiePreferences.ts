...
export const readCookiePreferences = (
  ...
  try {
    ...
    if (value.version !== 2 || typeof value.optionalAnalytics !== 'boolean') return null;
    return { version: 2, optionalAnalytics: value.optionalAnalytics };
  } catch {
    if (usesBrowserStorage) browserStorageFailed = true;
    return null;
  }
...
export const hasOutdatedCookiePreferences = (
  storage?: ReadableStorage | null,
): boolean => {
  try {
    ...
    const raw = resolvedStorage.getItem(COOKIE_PREFERENCES_STORAGE_KEY);
    if (!raw) return false;
    const value = JSON.parse(raw) as { version?: unknown; optionalAnalytics?: unknown };
    return value.version === 1 && typeof value.optionalAnalytics === 'boolean';
  } catch {
    return false;
  }
};
