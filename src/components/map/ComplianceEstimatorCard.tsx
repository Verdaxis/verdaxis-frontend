...
export const ComplianceEstimatorCard: React.FC<ComplianceEstimatorCardProps> = ({
    ...
    const selectedFuel = GREEN_FUEL_ASSUMPTIONS.find(fuel => fuel.marketProduct === selectedFuelProduct);
    const fuelOptions = useMemo(() => ACTIVE_MARKETPLACE_PRODUCT_OPTIONS.map(option => {
        const assumption = GREEN_FUEL_ASSUMPTIONS.find(fuel => fuel.marketProduct === option.value);
        return {
            value: option.value,
            label: option.label,
            description: assumption
                ? `${assumption.carbonIntensityGco2ePerMj} gCO2e/MJ · $${assumption.referencePriceUsdPerMt}/MT`
                : t('intelligencePanel.estimator.batchDataRequired'),
        };
    }), [t]);
    const marketplacePortOptions = useMemo(() => portOptions.map(port => {
        const availability = selectedFuelProduct === 'B30' || selectedFuelProduct === 'B100'
            ? 'unknown'
            : port.methanolSupply.toLowerCase();
        return {
            value: port.id,
            label: port.name,
            description: t('intelligencePanel.estimator.portBunkerDescription', {
                product: formatMarketProduct(selectedFuelProduct),
                availability: t(`intelligencePanel.availabilityLevels.${availability}`),
            }),
        };
    }), [portOptions, selectedFuelProduct, t]);
    ...
    const result = useMemo(() => selectedFuel ? estimateCompliancePlanning({
        ...DEFAULT_COMPLIANCE_ESTIMATOR_INPUT,
        voyageDays,
    ...
    }) : null, [conventionalPrice, dailyConsumption, etsCoveragePct, euaPrice, planningTarget, selectedFuel, voyageDays]);
    ...
    const resultSummary = result ? t('intelligencePanel.estimator.resultsSummary', {
        ...
        <section data-testid="compliance-estimator-card" className="rounded-lg border border-slate-100 bg-slate-50 dark:border-slate-700 dark:bg-slate-800/50">
            ...
            <div className="space-y-4 border-t border-slate-200 px-3 py-3 dark:border-slate-700">
                    ...
                    {result && selectedFuel ? (
                        ...
                        <div role="status" aria-live="polite" aria-atomic="true" className="rounded-md border border-slate-200 bg-white px-3 py-2 text-[11px] leading-relaxed text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
                            {t('intelligencePanel.estimator.messages.biodieselBatchDataRequired', { product: formatMarketProduct(selectedFuelProduct) })}
                        </div>
                    )}
