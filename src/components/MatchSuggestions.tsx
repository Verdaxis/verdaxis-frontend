            ...
            <div className="mb-3 bg-gradient-to-r from-emerald-500/5 to-blue-500/5 border border-dashed border-emerald-500/20 dark:border-emerald-500/10 rounded-lg p-4 text-center">
                {loading ? (
                    <>
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400">{t('matchSuggestions.loading')}</p>
                    </>
                ) : (
                    <>
                        <List size={20} className="mx-auto text-emerald-500/50 mb-2" />
                        <p className="text-xs text-slate-500 dark:text-slate-400">{t('matchSuggestions.empty')}</p>
                        {onNavigate && (
                            <button
                            ...
                            >
                                {t('matchSuggestions.openMarketplace')}
                            </button>
                        )}
                    </>
                )}
                ...
                return (
                    ...
                    >
                        <button
                            ...
                            aria-label={t('matchSuggestions.dismiss')}
                        ...
                        <div className="flex items-center gap-3 text-xs">
                            {askOrder && (
                                <div className="flex-1">
                                    <span className="text-slate-500 dark:text-slate-400">{t('matchSuggestions.ask')}: </span>
                                    <span className="font-bold text-slate-700 dark:text-slate-200">
                                        {askOrder.market_product ? formatMarketProduct(askOrder.market_product) : askOrder.fuel_type} · {Number(askOrder.remaining_quantity_mt).toLocaleString()} MT · ${Number(askOrder.price_per_mt_usd).toLocaleString()}/MT
                                    </span>
                                </div>
                            )}
                        </div>
                            ...
                            <div className="mt-1.5 flex flex-wrap gap-1">
                                {suggestion.match_reasons.slice(0, 3).map((reason, i) => (
                                    <span key={i} className="text-[10px] bg-white/50 dark:bg-slate-700/50 text-slate-500 dark:text-slate-400 px-1.5 py-0.5 rounded">
                                        {t(`matchSuggestions.reason.${reason}`, { defaultValue: t('matchSuggestions.reason.other') })}
                                    </span>
                                ))}
                            </div>
                        )}
