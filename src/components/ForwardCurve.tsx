...
export const ForwardCurve: React.FC<ForwardCurveProps> = ({ initialProductId, marketProduct, fuelType, deliveryPointName, onPeriodClick }) => {
    ...
    // When marketProduct or fuelType changes, auto-select the matching product
    useEffect(() => {
        if (products.length === 0) return;

        const exactMatch = marketProduct
            ? products.find((product) => product.market_product === marketProduct)
            : undefined;
        const familyMatch = fuelType
            ? products.find((product) => product.fuel_type.toLowerCase() === fuelType.toLowerCase())
            : undefined;
    ...
    }, [fuelType, marketProduct, products, selectedProductId]);
