                    ...
                    {trades.map(t => (
                        <div key={t.id} className="flex items-center justify-between px-4 py-1.5 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                            <div className="flex items-center gap-2 min-w-0">
                                <span className={`w-2 h-2 rounded-full flex-shrink-0 ${getDotColor(t.fuel_type)}`} />
                                <span className="text-xs font-medium text-slate-700 dark:text-slate-200 truncate">
                                    {shortFuel(t.fuel_type)}
                                    {t.fuel_grade && (
                                        <span className="ml-1 text-slate-400 text-[10px]">{gradeTag(t.fuel_grade)}</span>
                                    )}
                                </span>
                            </div>
                            <div className="flex items-center gap-3 flex-shrink-0">
                                <span className="text-xs font-mono text-slate-500 dark:text-slate-400">
                                    {t.quantity_mt.toLocaleString()} MT
                                </span>
                                <span className="text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400">
                                    ${t.price_per_mt_usd.toLocaleString()}/MT
                                </span>
                                <span className="text-[10px] text-slate-400 w-8 text-right">
                                    {relativeTime(t.confirmed_at)}
                                </span>
                            </div>
                        </div>
                    ))}
